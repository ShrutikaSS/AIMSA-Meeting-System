...
try {
    switch ($action) {
        ...
        case 'update_meeting':
            ...
            try {
                $notifText = "President rescheduled '{$title}' to {$meetingDate} at {$meetingTime} in {$venue}. Target: {$targetAudience}";
                $notifStmt = $pdo->prepare("INSERT INTO `notifications` (`title`, `text`, `indicator`, `recipient`) VALUES (?, ?, 'blue', ?)");
                $notifStmt->execute(["Meeting Rescheduled: {$title}", $notifText, $targetAudience]);
            } catch (Exception $ne) {
                error_log('President Meeting update notification error: ' . $ne->getMessage());
            }
